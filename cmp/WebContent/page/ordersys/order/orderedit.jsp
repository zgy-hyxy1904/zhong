...
$(function(){
	$("#submit").click(function(){
		$.ajax({
			url:"${pageContext.request.contextPath}/contorller/OrderContorller?flag=submitUpdate",
	...
	$("#save").click(function(){
		$.ajax({
			url:"${pageContext.request.contextPath}/contorller/OrderContorller?flag=saveUpdate",
			data:$("#saveForm").serialize(), 
			type:"post",
			success:function(data){
				if(data=='Y'){
    ...
    <div class="commonTitle">
        ...
        <form id="saveForm" name="saveForm" method="post">
		  <table border="0" cellspacing="1" cellpadding="0" class="commonTable">
			  <tr>
				...
				<td width="15%" align="left"><input type="text" style="width:150px" name = "orderId" value = "${order.orderId }"></td>
				<td width="10%" align="right" class="title"><span class="required">*</span>订单编码：</td>
				<td width="15%" align="left"><input type="text" style="width:150px" name = "orderCode" value = "${order.orderCode }"></td>
				<td width="10%" align="right" class="title"><span class="required">*</span>订单保存日期：</td>
				<td width="15%" align="left"><input type="text" style="width:150px" name = "orderDate" value = "<fmt:formatDate value="${order.orderDate }" pattern="yyyy-MM-dd"/>"></td>
				<td width="10%" align="right" class="title"><span class="required">*</span>订单状态：</td>
				<td width="15%" align="left">
					<select id="orderstatus" name = "orderFlag2">
		...
		<table width="90%" border="0" cellpadding="0" cellspacing="1" id="attachmentList" class="commonTable marginTopM">
	      ...
	      <c:forEach var = "o" items = "${order.orderDetails }">
		        <tr>
		            <td><input type='hidden' name='materId' value='${o.parts.partsId }'>${o.parts.partsName }</td>
		            <td align="center"><input type='text' name='count' value='${o.orderPartsCount }' class='inputTextNormal'></td>
		            <td>${o.parts.partsRepertory.partsRepCount }</td>
		            <td><button onclick="deleteRow('attachmentList',this);" class="btnIconDel" title="删除"></button></td>
		        </tr>
		      </c:forEach>
		</table>
		</form>
